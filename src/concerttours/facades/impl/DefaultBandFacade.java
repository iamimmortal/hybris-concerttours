...
public class DefaultBandFacade implements BandFacade
{
	...
	@Override
	public List<BandData> getBands(final String country)
	{
		final List<BandModel> bandModels = bandService.getBands(country);
	...
	@Override
	public int saveComment(final BandCommentModel bandCommentModel)
	{
		return bandService.save(bandCommentModel);
	}



	@Override
	public List<CommentData> getCommentForCode(final String code)
	{
		final List<BandCommentModel> commentModels = bandService.getCommentForCode(code);
		final List<CommentData> commentFacadeData = new ArrayList<>();
		for (final BandCommentModel sm : commentModels)
		{
			final CommentData sfd = new CommentData();
			sfd.setId(sm.getCode());
			sfd.setDate(sm.getDate().toString());
			sfd.setName(sm.getName());
			sfd.setComment(sm.getComment());
			commentFacadeData.add(sfd);
		}
		//Collections.sort(commentFacadeData, Collections.reverseOrder());
		return commentFacadeData;
	}
